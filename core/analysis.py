...
def run_analysis():
    """
    Fetch data lalu jalankan pure analysis.
    """

    entry_candles, err1 = get_binance_klines(
        interval=TF_ENTRY,
        limit=LIMIT,
    )

    htf1_candles, err2 = get_binance_klines(
        interval=TF_BIAS_1,
        limit=160,
    )

    htf2_candles, err3 = get_binance_klines(
        interval=TF_BIAS_2,
        limit=160,
    )

    if (
        entry_candles is None
        or htf1_candles is None
        or htf2_candles is None
    ):
        return {
            "error": err1 or err2 or err3 or "Data tidak lengkap"
        }

    return run_analysis_from_candles(
        entry_candles,
        htf1_candles,
        htf2_candles,
    )

def run_analysis_from_candles(
    entry_candles,
    htf1_candles,
    htf2_candles,
):
    ...
    Semua candle sudah disediakan caller.
    """
    ...
    if (
        len(entry_candles) < 130
        or len(htf1_candles) < 130
        or len(htf2_candles) < 130
    ):
        return {
            "error": "Data terlalu sedikit untuk analisa reliable (perlu >=130 candle untuk WMA119)"
        }
    ...
    if "BUY" in signal:
        ...
        retest_ok = get_retest_confirmation(
            entry_candles,
            "BUY",
            ob_bull,
            relevant_fvg,
        ...
        if not retest_ok:
            reasons = reasons + [
                "[DITAHAN] Belum ada retest valid ke zona OB/FVG"
            ]
    ...
    return {
        ...
        "wma_fast": round(float(wma_fast_val), 2)
        if not is_nan(wma_fast_val)
        else None,
        "wma_slow": round(float(wma_slow_val), 2)
        if not is_nan(wma_slow_val)
        else None,
